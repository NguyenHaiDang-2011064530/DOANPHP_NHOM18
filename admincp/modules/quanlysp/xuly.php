...
if(isset($_POST['themsanpham'])){
    ...
    if($hinhanh!=''){
        ...
        //xóa hình ảnh củ
        $sql ="SELECT * FROM tbl_sanpham WHERE id_sanpham = '$_GET[idsanpham]' LIMIT 1"; // Chỉnh sửa từ FORM thành FROM
    ...
    $id=$_GET['idsanpham'];
    $sql ="SELECT * FROM tbl_sanpham WHERE id_sanpham = '$id' LIMIT 1"; // Chỉnh sửa từ FORM thành FROM
    $query = mysqli_query($mysqli,$sql);
    while($row = mysqli_fetch_array($query)){
        unlink('uploads/' .$row['hinhanh']);
    }
